...
_Bool OneNet_DevLink(void)
{
	...
	OneNET_Authorization("2018-10-31", PROID, ET, ACCESS_KEY, DEVICE_NAME,
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	//每个变量之间除了最后不用加逗号之外都要加逗号
	memset(text, 0, sizeof(text));
//	UsartPrintf(USART1,"dev_U = %.2f\n",U);
	sprintf(text, "\"dev_U\":{\"value\":%.2f},", U);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
//	UsartPrintf(USART1,"dev_A = %.2f\n",A);
	sprintf(text, "\"dev_A\":{\"value\":%.2f},", A);
	strcat(buf, text);
	
    memset(text, 0, sizeof(text));
//	UsartPrintf(USART1,"dev_State = %s\n","false");
	sprintf(text, "\"dev_State\":{\"value\":%s},", RELAYS_STATE ? "true":"false");//"false","true"
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
//	UsartPrintf(USART1,"dev_Hour = %d\n",Hour);
	sprintf(text, "\"dev_Hour\":{\"value\":%d},", Hour);
	strcat(buf, text);

	memset(text, 0, sizeof(text));
//	UsartPrintf(USART1,"dev_Charg = %.2f\n",kWh);
	sprintf(text, "\"dev_Charg\":{\"value\":%.2f},", kWh);
	strcat(buf, text);

    memset(text, 0, sizeof(text));
//	UsartPrintf(USART1,"dev_Min = %d\n",Min);
	sprintf(text, "\"dev_Min\":{\"value\":%d}", Min);
	strcat(buf, text);
...
extern int APP_Hour;
extern int APP_Min;
unsigned char OneNet_FillBuf_APP_mode(char *buf)
{
	char text[50];	
	memset(text, 0, sizeof(text));
	strcpy(buf, "{\"id\":\"123\",\"params\":{");
...
//	UsartPrintf(USART1,"dev_State = %s\n","false");
	...
	memset(text, 0, sizeof(text));
//	UsartPrintf(USART1,"dev_Hour = %d\n",APP_Hour);
	sprintf(text, "\"dev_Hour\":{\"value\":%d},", APP_Hour);
	strcat(buf, text);
...
//	UsartPrintf(USART1,"dev_Charg = %.2f\n",kWh);
    ...
    memset(text, 0, sizeof(text));
//	UsartPrintf(USART1,"dev_Min = %d\n",APP_Min);
	sprintf(text, "\"dev_Min\":{\"value\":%d}", APP_Min);
	strcat(buf, text);
...
void OneNet_SendData_APP_mode(void)
{
	...
	memset(buf, 0, sizeof(buf));
	//封装数据包------上传的设备数据信息
	body_len = OneNet_FillBuf_APP_mode(buf);																	//获取当前需要发送的数据流的总长度
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
				...
				//提取关键信息
                relays_json = cJSON_GetObjectItem(params_json,"dev_State");
                if(relays_json != NULL){
					APP_Hour = 0;
					APP_Min = 0;
                    if(relays_json->type == cJSON_True){
                        RELAYS_ON;
						Tim3_Init(PER_1s*10,PSC_CONST);
                    }else{
                        RELAYS_OFF;
						TIM_DeInit(TIM3);
						APP_Return_flag = 1;
